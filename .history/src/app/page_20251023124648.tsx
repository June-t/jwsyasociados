...
import { createTimeline, stagger, splitText, onScroll } from "animejs";
import {
  IconArrowUpRight,
...
export default function Main() {
  useEffect(() => {
    // 1️⃣ Dividimos el texto del h2 por palabras con splitText()
    const { words } = splitText(".main__about h2", {
      lines: { wrap: "span" },
    });

    // 2️⃣ Creamos una timeline sincronizada con el scroll usando onScroll()
    const timeline = createTimeline({
      autoplay: onScroll({
        // Se activará cuando la sección entre al 70% del viewport
        target: document.querySelector(".main__about"),
        threshold: [1],
        debug: false, // cámbialo a true si quieres ver los indicadores de scroll
      }),
      defaults: {
        duration: 800,
        ease: "outExpo",
    ...
    // 3️⃣ Animamos las palabras del título con stagger
    timeline.add(
      words,
      {
        opacity: [0, 1],
    ...
    // 4️⃣ Animamos los demás elementos con fade y desplazamiento
    const aboutItems = document.querySelectorAll(
      ".main__about .about__content > *:not(h2)"
    );
